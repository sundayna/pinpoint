...
- 絕對不要用程式碼或程式語法
- 數學式用純文字表示，例如：x² 寫成 x^2，分數寫成 1/2，根號寫成 √
- 每個步驟說清楚「為什麼這樣做」，不只是「算出來是多少」
- 題目難度符合學測水準，不出超綱內容`
  }
};
...
function getUsage() {
  ...
  const stored = JSON.parse(localStorage.getItem('pinpoint_usage') || '{}');
...
function incrementUsage() {
  ...
  usage.count += 1;
  localStorage.setItem('pinpoint_usage', JSON.stringify(usage));
}
...
async function sendMessage() {
  ...
  if (getRemainingCount() <= 0) {
    appendMessage('ai', '⚠️ 你今天的免費次數（5次）已用完，明天再來繼續學習！\n\n🔜 會員功能即將推出，訂閱後可無限使用。');
    textarea.value = '';
    autoResize();
    return;
  }
