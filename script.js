...
auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    document.getElementById('user-btn').textContent = (user.email || '').split('@')[0];
    updateUsageDisplay();
    initChat();
  } else {
    currentUser = null;
...
async function loadUserData(user) {
  ...
  // 檢查 pending 升級
  const pendingSnap = await db.collection('pending_upgrades').doc(user.email).get();
  if (pendingSnap.exists) {
    ...
    await db.collection('pending_upgrades').doc(user.email).delete();
    isPremium = true;
    updateSubscribeBtn();
    return;
  }
  ...
  if (!snap.exists) {
    ...
    if (data.usage && data.usage.date !== today) {
      await ref.update({ usage: { date: today, count: 0 } });
    }
  }
  updateSubscribeBtn();
}
...
async function getRemainingCount() {
  ...
  const data = snap.data();
  const today = new Date().toDateString();
  if (!data.usage || data.usage.date !== today) return DAILY_LIMIT;
...
async function incrementUsage() {
  ...
  const data = snap.data();
  const today = new Date().toDateString();
  const count = (data.usage && data.usage.date === today) ? (data.usage.count || 0) : 0;
  await ref.update({ usage: { date: today, count: count + 1 } });
}
  ...
  try {
    if (isLoginMode) {
      ...
      if (!result.user.emailVerified) {
        await auth.signOut();
        errorEl.textContent = '請先驗證你的 Email，檢查信箱並點擊驗證連結';
        return;
      }
    } else {
      ...
      errorEl.textContent = '✅ 驗證信已寄出！請檢查信箱點擊連結後再登入';
    }
  } catch (err) {
    const msgs = {
    ...
    errorEl.textContent = msgs[err.code] || '發生錯誤：' + err.message;
  }
};
...
window.sendMessage = async function() {
  ...
  const remaining = await getRemainingCount();
  if (remaining <= 0) {
    appendMessage('ai', `⚠️ 你今天的免費次數（1次）已用完，明天再來繼續學習！`);
    textarea.value = '';
    autoResize();
